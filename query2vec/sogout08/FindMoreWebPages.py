...
for l in open('../ref/ntcir910.txt').readlines():
    docs.add(l.strip())
    print l.strip()
...
for f in files:
    if '.7z' in f:
        idx = int(f.split('.')[1]).__hash__()
        if idx%12==mod:
            doccount =0
            datacontent = os.popen('/usr/bin/7za e -so /work/caoyj/SogouT/'+f)
            ...
            while l !='':
                ...
                if '<DOCNO>' in l :
                    ...
                    if docno in docs:
                        ...
                        fout = open('../data/page/'+str(mod)+'.dat','a')
                        fout.write('<DOC>\n')
                        fout.write(l)
                        write = True
